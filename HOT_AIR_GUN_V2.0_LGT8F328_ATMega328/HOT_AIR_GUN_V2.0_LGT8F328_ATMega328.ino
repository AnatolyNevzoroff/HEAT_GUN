...
/////////////////////////////////////////////////////////////////////////////////////////////////
//Для вывода данных в серийный порт для настройки усилителя термопары раскомментируйте:
// #define LOG_ENABLE 
...
//ПОВЕРХНОСТИ ТЕРМОФЕНА БУДЕТ ВЫШЕ, ПРИМЕРНО 60 ГРАДУСОВ, ГОРЯЧО, НО ВПОЛНЕ БЕЗОПАСНО
...
#define MINIMUM_temp 100 //ГРАНИЦА МИНИМАЛЬНО-РЕГУЛИРУЕМОЙ ТЕМПЕРАТУРЫ ФЕНА 
...
//ОНО ЗАВИСИТ ОТ РАБОТЫ УСИЛИТЕЛЯ И ОПОРНОГО НАПРЯЖЕНИЯ, В НАШЕМ СЛУЧАЕ, ЭТО НАПРЯЖЕНИЕ ПИТАНИЯ
//НАПРИМЕР, АЦП LGT8F328 БОЛЬШЕ 1019 ПРИ ТЕОРЕТИЧЕСКОМ МАКСИМУМЕ 2^10(bit)=1024 НЕ ВЫДАЁТ, 
//ПОЭТОМУ ДЛЯ АВАРИЙНОГО ОТКЛЮЧЕНИЯ, ЗНАЧЕНИЕ "MAXIMUM_input_ADC" ВЫБРАНО НЕСКОЛЬКО МЕНЬШЕ,

//#define MAXIMUM_input_ADC 1014 //ЕСЛИ ПЛАТА РАБОТАЕТ В РЕЖИМЕ 2^12(bit)=4096 ВЫБИРАЕМ 4062

#define MAXIMUM_input_ADC 1020 //АРДУИНО НА БАЗЕ Atmega328p ВЫДАЛА МАКСИМУМ 1022
...
//ЕСЛИ ТЕМПЕРАТУРА ВЫШЕ Safe_HOT (ЗАДАННОЙ КАК БЕЗОПАСНАЯ) ЗНАЧИТ ЕСЛИ ТЭН ГОРЯЧИЙ, 
...
//ЕСЛИ ТЕМПЕРАТУРА ПАДАЕТ НИЖЕ Safe_HOT (ЗАДАННОЙ КАК БЕЗОПАСНАЯ) 
else{digitalWrite(FAN_out,LOW);//ОТКЛЮЧАЕМ ВЕНТИЛЯТОР
fanspeed=AirFlow;//ОТОБРАЖАЕМ ТОЧКАМИ ЗАДАННУЮ В МЕНЮ СКОРОСТЬ ВЕНТИЛЯТОРА
...
//ФЕН ОТКЛЮЧИТЬСЯ ВНЕ ЗАВИСИМОСТИ ОТ ТЕКУЩЕГО РЕЖИМА "РАБОТА" ИЛИ "ПАУЗА" 
//ЕСЛИ ВЫБРАТЬ СЛИШКОМ МАЛЕНЬКОЕ ЗНАЧЕНИЕ DownTime ОН МОЖЕТ ОТКЛЮЧИТЬСЯ ПРЯМО ВО ВРЕМЯ РАБОТЫ
//ЭТО НЕ ОШИБКА, А ДОПОЛНИТЕЛЬНАЯ ЗАЩИТА НА СЛУЧАЙ, ЕСЛИ ФЕН СЛУЧАЙНО ОКАЖЕТСЯ ВНЕ ДЕРЖАТЕЛЯ
...
fanspeed=AirFlow;dotfan();ar4=ar5=' ';LC.shutdown(0,false);tone(BUZZER_out,3000,100);
