...
//БИБЛИОТЕКА ПРОПОРЦИОНАЛЬНО-ИНТЕГРАЛЬНОГО ДИФФЕРЕНЦИРОВАНИЯ ДЛЯ ИНЕРТНЫХ СИСТЕМ (по указанному
//адресу лежит новая версия которая может вызвать ошибку, используйте версию из архива)
#include <LedControl.h>//https://github.com/wayoda/LedControl  \
БИБЛИОТЕКА ДЛЯ ДРАЙВЕРОВ MAX7219 И MAX7221 СВЕТОДИОДНЫХ ИНДИКАТОРОВ И ПАНЕЛЕЙ

/////////////////////////////////////////////////////////////////////////////////////////////////
//Для вывода данных в серийный порт раскомментируйте инструкцию: "#define SERIAL_ENABLE"
// #define SERIAL_ENABLE 
//ПЕРЕД ОКОНЧАТЕЛЬНОЙ КОМПИЛЯЦИЕЙ ЗАКОММЕНТИРОВАТЬ ИЛИ УДАЛИТЬ!
/////////////////////////////////////////////////////////////////////////////////////////////////

//Для ПЕРВОНАЧАЛЬНОГО запуска термофена необходимо загрузить в память МК рекомендованные 
//параметры, для этого раскомментируйте инструкцию "#define START_VALUE" и запишите скетч в МК
// #define START_VALUE
//Кнопкой "POWER" включите и выключите термофен, (параметры сохраняться в EEPROM МК)
//ЗАКОММЕНТИРУЙТЕ или УДАЛИТЕ инструкцию "#define START_VALUE" и ПОВТОРНО запишите скетч в МК

//Для настройки усилителя термопары без экрана раскомментируйте:
// #define LOG_ENABLE 
//ПЕРЕД ОКОНЧАТЕЛЬНОЙ КОМПИЛЯЦИЕЙ ЗАКОММЕНТИРОВАТЬ ИЛИ УДАЛИТЬ!

//Для ПРОВЕРКИ функции прерывания и работы кросс-детектора раскомментируйте:
// #define ATTACH_INTERRUPT 
//ПЕРЕД ОКОНЧАТЕЛЬНОЙ КОМПИЛЯЦИЕЙ ЗАКОММЕНТИРОВАТЬ ИЛИ УДАЛИТЬ!
...
//ПОВЕРХНОСТИ ТЕРМОФЕНА БУДЕТ ВЫШЕ, ПРИМЕРНО 60 ГРАДУСОВ, ГОРЯЧО, НО ДОСТАТОЧНО БЕЗОПАСНО
...
#define MINIMUM_temp 80 //ГРАНИЦА МИНИМАЛЬНО-РЕГУЛИРУЕМОЙ ТЕМПЕРАТУРЫ ФЕНА 

#define MAXIMUM_input_ADC 1024 //ЭТО ЗНАЧЕНИЕ ТРЕБУЕТ КОРРЕКТИРОВКИ!!! ЧИТАЕМ READ_MY !
...
//МАССИВ ЧАСТОТ ДЛЯ ФУНКЦИИ tone() (при желании корректируем)
int16_t Frequency[5]={0,2077,1577,1077,577};

//byte от 0 до 255
uint8_t temp00,temp01,KpSet10,KpSet11,KiSet10,KiSet11,KdSet10,KdSet11;
uint8_t cold,AirFlow,AirFlowMOD,fanspeed,StepValue,StepValueMOD,LED_Bright,LED_BrightMOD;
...
//МАССИВ ЗНАЧЕНИЙ ШИМ ДЛЯ СКОРОСТИ ВЕНТИЛЯТОРА (при желании корректируем)
...
//digitalWrite(BUZZER_out,LOW);//ПЬЕЗОПИЩАЛКА (зависит от типа баззера)
...
//РЕКОМЕНДОВАННЫЕ ЗНАЧЕНИЯ ПЕРЕМЕННЫХ
#ifdef START_VALUE
SET_Temp=150;KpSet=240;KiSet=40;KdSet=0;AirFlow=4;LEDshow=1;
StepValue=10;WaitAutoRet=20;DownTime=25;LED_Bright=1;
#endif
...
timer_BTN=mill+p50;}//УСТАНАВЛИВАЕМ АНТИДРЕБЕЗГОВУЮ ЗАДЕРЖКУ 50мс timer_POWER_key

if(FL_delay_POWER_key==true&&mill>timer_BTN){//ЕСЛИ СПУСТЯ 50мс 
if(POWER_btn==LOW){//ЕСЛИ СОСТОЯНИЕ КНОПКИ POWER НЕ ИЗМЕНИЛОСЬ
PowerStatus=!PowerStatus;FL_start_POWER=true;//ВЫПОЛНЯЕМ ДЕЙСТВИЯ ДЛЯ КНОПКИ
}
...
for(INDEX_Freq=5;INDEX_Freq>0;INDEX_Freq--){tone(BUZZER_out,Frequency[INDEX_Freq],p100);
delay(p100);}//noTone(BUZZER_out);
digitalWrite(BUZZER_out,HIGH);//ОТКЛЮЧАЕМ ЗВУКОВОЙ СИГНАЛ
delay(p200);//ПАУЗА ПЕРЕД ВКЛЮЧЕНИЕМ
...
timer_GERKON=mill+p200;}//УСТАНАВЛИВАЕМ АНТИДРЕБЕЗГОВУЮ ЗАДЕРЖКУ 200мс (ГЕРКОН ПЛОХАЯ КНОПКА)
...
//ЕСЛИ ТЕМПЕРАТУРА ВЫШЕ Safe_HOT (ЗАДАННОЙ КАК БЕЗОПАСНАЯ) ЗНАЧИТ ТЭН ГОРЯЧИЙ! 
//ВКЛЮЧАЕМ ВЕНТИЛЯТОР ДЛЯ ОХЛАЖДЕНИЯ, СТУПЕНЧАТО ПОНИЖАЯ СКОРОСТЬ В ЗАВИСИМОСТИ ОТ ТЕМПЕРАТУРЫ
if(FL_start_COOLING==true&&Input_TEMP>Safe_HOT){
cold=map(Input_TEMP,Safe_HOT,temp_for_FAN,1,8);//ЧЕМ НИЖЕ ТЕМПЕРАТУРА, ТЕМ МЕНЬШЕ СКОРОСТЬ 
cold=constrain(cold,1,8);//УСТАНАВЛИВАЕМ СКОРОСТЬ В ДИАПАЗОНЕ 1-8 
analogWrite(FAN_out,pwm[cold]);//ИЗ МАССИВА pwm ВЫБИРАЕМ ЗНАЧЕНИЕ СООТВЕТСТВУЮЩЕЕ СКОРОСТИ
fanspeed=cold;}//ОТОБРАЖАЕМ ТОЧКАМИ СКОРОСТЬ ВЕНТИЛЯТОРА ПРИ ОХЛАЖДЕНИИ, А НЕ ЗАДАННУЮ В МЕНЮ

//ЕСЛИ ТЕМПЕРАТУРА ПАДАЕТ НИЖЕ Safe_HOT (ЗАДАННОЙ КАК БЕЗОПАСНАЯ) ЗНАЧИТ ТЭН БЕЗОПАСЕН
else{digitalWrite(FAN_out,LOW);//ОТКЛЮЧАЕМ ВЕНТИЛЯТОР
fanspeed=AirFlow;//ОТОБРАЖАЕМ ТОЧКАМИ ЗАДАННУЮ В МЕНЮ РАБОЧУЮ СКОРОСТЬ ВЕНТИЛЯТОРА
FL_start_COOLING=false;}//БЛОКИРУЕМ ВКЛЮЧЕНИЕ ВЕНТИЛЯТОРА
//ДАЖЕ ЕСЛИ ТЕМПЕРАТУРА НЕЗНАЧИТЕЛЬНО ПРЕВЫСИТ Safe_HOT, НО ОСТАНЕТСЯ ДОПУСТИМО БЕЗОПАСНОЙ
...
//RISING это режим обработки прерывания. Всего доступны следующие режимы:
...
//Если сигнал с "АЦП" превысил максимально-допустимое значение, СРАЗУ УХОДИМ В "POWER OFF"
//Сработает, только если вы правильно выставили значение MAXIMUM_input_ADC (читаем READ_MY)
...
if(FL_start_MENU==true){//ОТКЛЮЧАЕМ ЗВУКОВОЙ СИГНАЛ
//tone(BUZZER_out,3000,80);//noTone(BUZZER_out);digitalWrite(BUZZER_out,HIGH);
m0=0;ar4=ar5=' ';fanspeed=AirFlow;timer_MENU=mill-p1000;FL_ShowMenu=true;
   FL_start_MENU=false;}
...
case 0:trig0=trig0+(newPos*StepValue);
    if(trig0>=MINIMUM_temp){SET_Temp=constrain(trig0,MINIMUM_temp,MAX_temp-30);}
...
//ВЫВОДИМ ЗНАЧЕНИЯ НА ИНДИКАТОР 1 РАЗ В СЕКУНДУ (чаще нет необходимости)
...
//ОТОБРАЖАЕМ СКОРОСТЬ ВЕНТИЛЯТОРА В ВИДЕ ТОЧЕК
dotfan();

//ОТОБРАЖАЕМ ФАКТИЧЕСКУЮ ТЕМПЕРАТУРУ И ЗАДАННУЮ ТЕМПЕРАТУРЫ ИЛИ СКОРОСТЬ ВЕНТИЛЯТОРА
ShowMENU();}


//ЧЕРЕЗ ЗАДАННЫЙ ИНТЕРВАЛ ПЕРЕКЛЮЧАЕМ ФЕН В РЕЖИМ "STANDBY"
if(mill-timer_autoret_DOWN>DownTime*p60000){
//ДЛЯ ДОПОЛНИТЕЛЬНОЙ ЗАЩИТЫ, НА СЛУЧАЙ, ЕСЛИ ФЕН СЛУЧАЙНО ОКАЖЕТСЯ ВНЕ ДЕРЖАТЕЛЯ (УПАДЁТ И
//И БУДЕТ РАБОТАТЬ ПОКА НЕ ЗАКОНЧИТСЯ ВРЕМЯ УСТАНОВЛЕННОЕ ТАЙМЕРОМ АВТОМАТИЧЕСКОГО ОТКЮЧЕНИЯ)
//ФЕН НЕОБХОДИМО ОТКЛЮЧАТЬ ВНЕ ЗАВИСИМОСТИ ОТ ТЕКУЩЕГО РЕЖИМА "РАБОТА" ИЛИ "ПАУЗА" 
//ДЛЯ ЭТОГО НЕОБХОДИМО СТРОКУ:
if(GERKON_flag=false){PowerStatus=false;FL_start_POWER=true;}}
//ЗАМЕНИТЬ НА: PowerStatus=false;FL_start_POWER=true;}
//НО В ТАКОМ СЛУЧАЕ, ЕСЛИ ВЫБРАТЬ СЛИШКОМ МАЛЕНЬКОЕ ЗНАЧЕНИЕ DownTime 
//ФЕН БУДЕТ ОТКЛЮЧИТЬСЯ (УХОДИТЬ В "STANDBY") ПРЯМО ВО ВРЕМЯ РАБОТЫ.
...
ar2=ar3=ar4=ar5=ar6=ar7=' ';//ЧИСТИМ ПОЗИЦИИ ДО ОТОБРАЖЕНИЯ МЕНЮ НАСТРОЙКИ
...
tone(BUZZER_out,3000,100);//ПОДАЁМ КОРОТКИЙ СИГНАЛ АКТИВАЦИИ МЕНЮ НАСТРОЙКИ ПАРАМЕТРОВ
//noTone(BUZZER_out);
digitalWrite(BUZZER_out,HIGH);//ОТКЛЮЧАЕМ ЗВУКОВОЙ СИГНАЛ
   FL_start_MENU=false;}
...
m1=m1+newPos;if(m1>6){m1=0;}if(m1<0){m1=6;}ar2=ar3=ar4=ar5=ar6=ar7=' ';
...
tone(BUZZER_out,3000,100);//КОРОТКИЙ СИГНАЛ ВОЗВРАТА В ГЛАВНОЕ МЕНЮ
//noTone(BUZZER_out);
digitalWrite(BUZZER_out,HIGH);//ОТКЛЮЧАЕМ ЗВУКОВОЙ СИГНАЛ
...
//noTone(BUZZER_out);digitalWrite(BUZZER_out,HIGH);//ОТКЛЮЧАЕМ ЗВУКОВОЙ СИГНАЛ
...
if(FL_start_MENU==true){
dot0();ar1=ar7=ar8=' ';triac=0;TEMPERATURA();
...
Serial.print("pwm[cold] ");Serial.println(pwm[cold]);
#endif
...
cold=map(Input_TEMP,Safe_HOT,SET_Temp,1,8);
cold=constrain(cold,1,8);
analogWrite(FAN_out,pwm[cold]);}
...
fanspeed=0;dotfan();ar4=ar5=' ';LC.shutdown(0,false);
tone(BUZZER_out,3000,100);//noTone(BUZZER_out);
digitalWrite(BUZZER_out,HIGH);
attachInterrupt(0,tracking,RISING);triac=0;analogWrite(FAN_out,pwm[4]);EncodCL();
   FL_start_MENU=false;}
...
ar1=' ';
ar2=Input_TEMP/100%10;if(Input_TEMP<100){ar2=' ';}
ar3=Input_TEMP/10%10;
ar4=Input_TEMP%10;}

//ВЫВОДИМ ПРОЦЕНТ ЗАПОЛНЕНИЯ ШИМ НАПРЯМУЮ (БЕЗ ПИД) ДЛЯ НАГРЕВА ТЭН
ar6=(triac)/100%10;if(ar6==0){ar6=' ';}
ar7=(triac)/10%10;
ar8=(triac)%10;
ShowMENU();}
}//END ENGINEERING MENU 
