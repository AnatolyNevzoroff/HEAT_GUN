...
else{digitalWrite(FAN_out,LOW);fanspeed=fan;//ПОКАЗЫВАЕМ ЗАДАННУЮ СКОРОСТЬ ВЕНТИЛЯТОРА
FL3=false;}
...
if(FL5==true){//ЗАПОЛНЯЕМ ПУСТЫМИ СИМВОЛАМИ НЕИСПОЛЬЗУЕМЫЕ В ДАННОМ МЕНЮ СЕГМЕНТЫ ИНДИКАТОРА
...
//ЭНКОДЕРОМ МЕНЯЕМ ЗНАЧЕНИЯ ТЕМПЕРАТУРЫ ИЛИ ПОТОК ВОЗДУХА 
if(newPos!=oldPos){
switch (m0){
...
//ВЫВОДИМ ПАРАМЕТР ДЛЯ ФУНКЦИИ tracking ЗАДАВАЕМЫЙ НАПРЯМУЮ (БЕЗ ПИД) ДЛЯ НАГРЕВА ТЭН
