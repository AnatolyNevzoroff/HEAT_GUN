...
#define hot 44 //ТЕМПЕРАТУРА ОТКЛЮЧЕНИЯ ВЕНТИЛЯТОРА ПРИ ОХЛАЖДЕНИИ ТЭНа ПРИ ВОЗВРАТЕ НА БАЗУ
//ФАКТИЧЕСКАЯ ТЕМПЕРАТУРА СТАЛЬНОЙ ПОВЕРХНОСТИ ТЕРМОФЕНА В МОМЕНТ ОТКЛЮЧЕНИЯ, БУДЕТ ВЫШЕ!
...
//АВАРИЙНОГО ОТКЛЮЧЕНИЯ, "maxin" ВЫБРАНО НЕСКОЛЬКО МЕНЬШЕ, НАПРИМЕР: 4062 ИЛИ 1060
#define maxin 4060 //В ЛЮБОМ СЛУЧАЕ, ЗНАЧЕНИЕ ДОЛЖНО БЫТЬ МЕНЬШЕ: 2^12(bit)=4096
...
int16_t triac,inputdata,temp,tempMOD,In_TEMP,temp_for_FAN,
KpSet,KpSetMOD,KiSet,KiSetMOD,KdSet,KdSetMOD,trig0,trig1;
...
uint8_t cold,fan,fanMOD,fanspeed,sharp,sharpMOD,bright,brightMOD;
...
uint8_t pwm[9]={0,100,109,119,127,138,150,165,255};

//char от -128 до 127
int8_t fr,m0,m1,m2,menu,down,downMOD,wait,waitMOD,oldPos,newPos;

//boolean от false до true
bool power,led,ledMOD,GERKON_flag,btn,POWER_btn,GERKON_btn,MENU_btn;
...
LC.setIntensity(0,bright);//УСТАНАВЛИВАЕМ ИНТЕНСИВНОСТЬ СВЕЧЕНИЯ (0 ... 15)
...
FL0=true;//ПОДНИМАЕМ ФЛАГ ЧТОБ ЗАДАТЬ ПАРАМЕТРЫ ДЛЯ РЕЖИМА "STANDBY"
}//END SETUP 
...
if(btn!=POWER_btn){delay(50);btn=digitalRead(POWER_key);
...
timer_BUZZER=mill+p200;//ЗАДЕРЖКА ПОДАЧИ МНОГОТОНАЛЬНОГО ЗВУКОВОГО СИГНАЛА ВКЛЮЧЕНИЯ 
//ЗАПИСЫВАЕМ ЗНАЧЕНИЯ ВРЕМЕННЫХ ПЕРЕМЕННЫХ ДЛЯ СРАВНЕНИЯ ПЕРЕД ЗАПИСЬЮ
tempMOD=temp,KiSetMOD=KiSet,KpSetMOD=KpSet,KdSetMOD=KdSet,fanMOD=fan,ledMOD=led,
sharpMOD=sharp,waitMOD=wait,downMOD=down,brightMOD=bright;
FL0=false;}
...
btn=digitalRead(GERKON_key);//ОПРАШИВАЕМ СОСТОЯНИЕ ВХОДА С ПОДКЛЮЧЁННЫМ ГЕРКОНОМ
if(btn!=GERKON_btn){//ЕСЛИ СОСТОЯНИЕ ГЕРКОНА ИЗМЕНИЛОСЬ
GERKON_btn=btn;//ЗАПИСЫВАЕМ ВРЕМЕННОЕ СОСТОЯНИЕ ГЕРКОНА КАК ТЕКУЩЕЕ
FL9=true;//ПОДНИМАЕМ ФЛАГ 
timer_GERKON=mill+p200;}//УСТАНАВЛИВАЕМ АНТИДРЕБЕЗГОВУЮ ЗАДЕРЖКУ 200мс 


if(FL9==true&&mill>timer_GERKON){//ЕСЛИ СПУСТЯ 200мс СОСТОЯНИЕ ГЕРКОНА ТАК И НЕ ИЗМЕНИЛОСЬ 
if(GERKON_flag!=GERKON_btn){//ПРИ ЭТОМ ОНО НЕ СООТВЕТСТВУЕТ ТЕКУЩЕМУ СОСТОЯНИЮ ФЛАГА ДЛЯ РУЧКИ
if(GERKON_btn){GERKON_flag=true;}//РУЧКА ВНЕ БАЗЫ И ФЕН В РАБОТЕ
else{GERKON_flag=false;}//РУЧКА НА БАЗЕ И ФЕН НА ПАУЗЕ
FL2=true;FL5=true;clmil();}
FL9=false;}
...
temp_for_FAN=temp;//if(temp_for_FAN<minimtemp){temp_for_FAN=minimtemp;}
FL3=true;//ПОДНИМАЕМ ФЛАГ ДЛЯ ВКЛЮЧЕНИЯ ВЕНТИЛЯТОРА ОХЛАЖДЕНИЯ
FL2=false;}


//ЕСЛИ ТЭН ГОРЯЧИЙ, ВКЛЮЧАЕМ ВЕНТИЛЯТОР ДЛЯ ОХЛАЖДЕНИЯ СО СКОРОСТЬЮ ЗАВИСЯЩЕЙ ОТ ТЕМПЕРАТУРЫ
if(FL3==true&&In_TEMP>hot){
cold=map(In_TEMP,hot,temp_for_FAN,1,8);//ЧЕМ НИЖЕ ТЕМПЕРАТУРА, ТЕМ МЕНЬШЕ СКОРОСТЬ 
cold=constrain(cold,1,8);analogWrite(FAN_out,pwm[cold]);
fanspeed=cold;}//ТОЧКАМИ ПОКАЗЫВАЕМ СКОРОСТЬ ВЕНТИЛЯТОРА ПРИ ОХЛАЖДЕНИИ
//ЕСЛИ ТЕМПЕРАТУРА ПАДАЕТ НИЖЕ БЕЗОПАСНОЙ ОТКЛЮЧАЕМ ВЕНТИЛЯТОР
else{digitalWrite(FAN_out,LOW);fanspeed=fan;//УСТАНОВЛЕННАЯ СКОРОСТЬ ВЕНТИЛЯТОРА
FL3=false;}


//ПОДМИГИВАЕМ ИНДИКАТОРОМ "POWER" СООБЩАЯ, ЧТО ФЕН ВКЛЮЧЕН, НАХОДИТСЯ НА БАЗЕ, НО В РЕЖИМЕ "ПАУЗА"
if(mill-timer_LED>(FL8?p1000:p100)){timer_LED=mill;FL8=!FL8;digitalWrite(POWER_LED_out,FL8);}
...
if(FL5==true){
m0=0;ar4=ar5=' ';fanspeed=fan;timer_MENU=mill-p500;
FL5=false;}
...
if(btn==LOW&&MENU_btn==HIGH){++m0;if(m0>1){m0=0;}
timer_BTN=mill;timer_MENU=mill-p500;FL4=true;clmil();}
MENU_btn=btn;}
...
//МЕНЯЕМ ЭНКОДЕРОМ ЗНАЧЕНИЯ ТЕМПЕРАТУРЫ ИЛИ ПОТОК ВОЗДУХА 
if(newPos!=oldPos){
switch (m0){
case 0:trig0=trig0+(newPos*sharp);if(trig0>=minimtemp){temp=constrain(trig0,minimtemp,maxtemp);}
else{if(trig0<temp){trig0=0;}else{trig0=minimtemp;}temp=trig0;};break;
case 1:trig0=trig0+newPos;fan=constrain(trig0,1,8);break;}
clful();fanspeed=fan;timer_MENU=mill-p500;}
...
dotfan();menu0();}//ВЫЗЫВАЕМ ФУНКЦИИ ОТОБРАЖЕНИЯ СКОРОСТИ В ВИДЕ ТОЧЕК И ЦИФРЫ ЗНАЧЕНИЙ
...
FL6=true;dot8();//ПО УМОЛЧАНИЮ РЕЖИМ ПРОКРУТКИ ПУНКТОВ МЕНЮ
...
if(FL6==true){dot8();}//ДЛЯ ПРОКРУТКИ ПУНКТОВ МЕНЮ ЗАЖИГАЕМ ВСЕ ТОЧКИ
if(FL6==false){dot0();}}//ДЛЯ РЕДАКТИРОВАНИЯ (ИЗМЕНЕНИЯ ПАРАМЕТРОВ) ГАСИМ ВСЕ ТОЧКИ
MENU_btn=btn;}
...
if(FL4==true&&MENU_btn==LOW&&mill-timer_BTN>p500){
menu++;if(menu>1){menu=0;}FL5=true;
FL4=false;}
...
if(FL0==true){
fr=0;menu=0;FL5=true;LC.clearDisplay(0);EEPROM_UPDATE();temperatura();
FL0=false;}
...
timer_BTN=mill+p1000;FL4=true;timer_MENU=mill-p1000;}
MENU_btn=btn;}
...
if(FL3==true&&In_TEMP>hot){temperatura();
...
if(mill-timer_MENU>p500){timer_MENU=mill;
if(m2==0){
//ВЫВОДИМ РЕЗУЛЬТАТ АЦП 0...4096 ОЦИФРОВКИ НАПРЯЖЕНИЯ С УСИЛИТЕЛЯ, ЛИБО... 
