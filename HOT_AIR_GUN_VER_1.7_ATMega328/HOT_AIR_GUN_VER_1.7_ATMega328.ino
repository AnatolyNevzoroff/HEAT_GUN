...
//НАПРЯЖЕНИЕ ПИТАНИЯ АРДУИНО) НАПРИМЕР, У МЕНЯ АЦП БОЛЬШЕ 1023 НЕ ВЫДАЁТ, ПОЭТОМУ ДЛЯ 
//АВАРИЙНОГО ОТКЛЮЧЕНИЯ, "maxin" ВЫБИРАНО НЕСКОЛЬКО МЕНЬШЕ, НАПРИМЕР: 1021 ИЛИ 1022
#define maxin 1021 //В ЛЮБОМ СЛУЧАЕ, ЗНАЧЕНИЕ ДОЛЖНО БЫТЬ МЕНЬШЕ: 1024
...
int16_t triac,inputdata,temp,In_TEMP,temp_for_FAN,KpSet,KiSet,KdSet,trig0,trig1;
...
bool power,led,GERKON_flag,btn,POWER_btn,GERKON_btn,MENU_btn;//ENC_BTN_flag,
...
//ПЕРЕВОДИМ ПИНЫ D9 и D10 ТАЙМЕРА №1 НА ЧАСТОТУ - 31.4 кГц (УВОДИМ ШИМ ИЗ СЛЫШИМОЙ ОБЛАСТИ)
...
//ЕСЛИ ЗНАЧЕНИЯ С ПОРТА "А0" ОЧЕВИДНО ЗАВЫШЕНЫ, СРАЗУ УХОДИМ В "POWER OFF"
...
timer_GERKON=mill+p200;}//ЕСЛИ СОСТОЯНИЕ ГЕРКОНА ИЗМЕНИЛОСЬ, УСТАНАВЛИВАЕМ ЗАДЕРЖКУ 200мс 
if(FL9==true&&mill>timer_GERKON){//ЕСЛИ СПУСТЯ 200мс РУЧКА ПРОДОЛЖАЕТ НАХОДИТЬСЯ ВНЕ БАЗЫ 
...
tone(BUZZER_out,3000,100);//КОРОТКИЙ СИГНАЛ ВХОДА В МЕНЮ
FL5=false;}
...
if(FL6==true){dot8();}//ДЛЯ ПРОКРУТКИ ПУНКТОВ МЕНЮ ЗАЖИГАЕМ ТОЧКИ
if(FL6==false){dot0();}}//ДЛЯ РЕДАКТИРОВАНИЯ (ИЗМЕНЕНИЯ ПАРАМЕТРОВ) ТОЧКИ ГАСИМ
MENU_btn=btn;}
